...
function TopStatsCards(props: TopStatsProps) {
    return (
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8 mb-10">
            ...
            <div className="relative z-20 group">
                ...
                <div className="p-4 sm:p-6 flex flex-col border border-white/5 rounded-2xl relative z-10 shadow-inner h-full" style={{ background: 'radial-gradient(ellipse at top, rgba(21,27,39,0.95) 0%, rgba(21,27,39,0.90) 100%)' }}>
                    <div className="flex justify-between items-start mb-3 sm:mb-4">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-400 border border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.2)] text-sm sm:text-base">📊</div>
                        <span className="text-[9px] sm:text-[10px] font-mono text-emerald-400 bg-emerald-500/10 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border border-emerald-500/20 tracking-widest">GLOBAL</span>
                    </div>
                    <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Total Volume</p>
                    <h3 className="text-xl sm:text-2xl font-black text-white font-mono">{props.totalDisbursed} <span className="text-xs sm:text-sm text-slate-500 font-sans font-bold">αUSD</span></h3>
                </div>
            </div>
            ...
            <div className="relative z-20 group">
                ...
                <div className="p-4 sm:p-6 flex flex-col border border-white/5 rounded-2xl relative z-10 shadow-inner h-full" style={{ background: 'radial-gradient(ellipse at top, rgba(21,27,39,0.95) 0%, rgba(21,27,39,0.90) 100%)' }}>
                    <div className="flex justify-between items-start mb-3 sm:mb-4">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-cyan-500/10 flex items-center justify-center text-cyan-400 border border-cyan-500/20 shadow-[0_0_10px_rgba(6,182,212,0.2)] text-sm sm:text-base">🏦</div>
                        <select aria-label="Select vault token" value={props.activeVaultToken.symbol} onChange={(e) => { const found = props.SUPPORTED_TOKENS.find((t: any) => t.symbol === e.target.value); if (found) props.setActiveVaultToken(found); }} className="text-[9px] sm:text-[10px] font-mono font-bold text-cyan-400 bg-cyan-500/10 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border border-cyan-500/30 outline-none cursor-pointer hover:bg-cyan-500/20 transition-colors">
                            {props.SUPPORTED_TOKENS.map((t: any) => <option key={t.symbol} value={t.symbol}>{t.symbol}</option>)}
                        </select>
                    </div>
                    <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Vault Balance</p>
                    <h3 className="text-xl sm:text-2xl font-black text-white font-mono">{props.vaultBalance} <span className="text-xs sm:text-sm text-cyan-500 font-sans font-bold">{props.activeVaultToken.symbol}</span></h3>
            ...
            <div className="relative z-20 group">
                ...
                <div className="p-4 sm:p-6 flex flex-col border border-white/5 rounded-2xl relative z-10 shadow-inner h-full" style={{ background: 'radial-gradient(ellipse at top, rgba(21,27,39,0.95) 0%, rgba(21,27,39,0.90) 100%)' }}>
                    <div className="flex justify-between items-start mb-3 sm:mb-4">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-fuchsia-500/10 flex items-center justify-center text-fuchsia-400 border border-fuchsia-500/20 shadow-[0_0_10px_rgba(217,70,239,0.2)] text-sm sm:text-base">🤖</div>
                        <span className="text-[9px] sm:text-[10px] font-mono text-fuchsia-400 bg-fuchsia-500/10 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border border-fuchsia-500/20 tracking-widest">AUTOPILOT</span>
                    </div>
                    <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Active Agents</p>
                    <h3 className="text-xl sm:text-2xl font-black text-white font-mono">{props.activeBotsCount} <span className="text-xs sm:text-sm text-slate-500 font-sans font-bold">Bots</span></h3>
                </div>
            </div>
            ...
            <div className="relative z-20 group">
                ...
                <div className="p-4 sm:p-6 flex flex-col border border-white/5 rounded-2xl relative z-10 shadow-inner h-full" style={{ background: 'radial-gradient(ellipse at top, rgba(21,27,39,0.95) 0%, rgba(21,27,39,0.90) 100%)' }}>
                    <div className="flex justify-between items-start mb-3 sm:mb-4">
                        <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border shadow-[0_0_10px_rgba(0,0,0,0.2)] text-sm sm:text-base ${props.agentStatus === 'ACTIVE' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                            {props.agentStatus === 'ACTIVE' ? '⚡' : '⏸'}
                        </div>
                        <span className={`text-[9px] sm:text-[10px] font-mono px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border tracking-widest ${props.agentStatus === 'ACTIVE' ? 'text-amber-400 bg-amber-500/10 border-amber-500/20 animate-pulse' : 'text-rose-400 bg-rose-500/10 border-rose-500/20'}`}>
                            {props.agentStatus}
                        </span>
                    </div>
                    <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Daemon Engine</p>
                    <div className="flex justify-between items-end mt-auto">
                        <h3 className="text-lg sm:text-xl font-black text-white font-sans">{props.agentStatus === 'ACTIVE' ? 'Processing' : 'Halted'}</h3>
                        {props.isAdmin && (
