    ...
    return (
        <div className="mt-2 flex gap-4 overflow-x-auto pb-6 cyber-scroll-x animate-in slide-in-from-top-4 fade-in duration-500 relative z-10">
            {liveIntents.map((intent) => {
                ...
                const statusColor = needsWallet && !isWalletLocked ? 'text-amber-500' : 'text-emerald-500';
                const dotColor = needsWallet && !isWalletLocked ? 'bg-amber-500' : 'bg-emerald-500';

                return (
                    <div key={intent.indexId}
                        className={`relative min-w-[340px] p-5 rounded-2xl border shadow-lg ${cardBorderClass} group/card`}
                    ...
                    >
                        ...
                        {handleDeleteIntent && (
                            <button
                                ...
                                className="absolute top-3 right-3 w-6 h-6 flex items-center justify-center rounded-lg hover:bg-rose-500/20 transition-all opacity-0 group-hover/card:opacity-100 z-10"
                                ...
                                {showAliasInput ? (
                                    aliasLocked ? (
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <h4 className="text-lg font-black tracking-wide leading-tight truncate" style={{ color: 'var(--pp-text-primary)' }}>
                                            ...
                                            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest border shrink-0
                                                ...
                                                style={!displayAlias ? { color: 'var(--pp-text-muted)' } : {}}
                                            >
                                        ...
                                        <div className="flex flex-col gap-1.5 w-full">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-[9px] font-bold tracking-[0.15em] text-amber-500 uppercase">Unrecognized Address</span>
                                            </div>
                                            <div className="relative flex items-center">
                                                <input
                                                    ...
                                                    className="w-full border border-emerald-500/20 hover:border-emerald-500/35 focus:border-emerald-400/55 rounded-lg pl-3 pr-[3.75rem] py-[7px] text-sm focus:outline-none font-mono tracking-wide transition-all duration-200"
                                                ...
                                                <button
                                                    ...
                                                    className={`absolute right-1.5 top-1/2 -translate-y-1/2 text-[9px] font-bold uppercase tracking-widest px-2 py-[5px] rounded-md border transition-all duration-200
                                                        ${displayAlias
                                                            ? 'text-emerald-500 bg-emerald-500/15 border-emerald-500/40 hover:bg-emerald-500/25 hover:border-emerald-400/70'
                                            ...
                                            <p className="text-[9px] font-mono leading-none">
                                                {displayAlias
                                                    ...
                                                    : <span style={{ color: 'var(--pp-text-muted)' }}>{'\u21B5'} Enter or Skip to leave anonymous</span>}
                                            </p>
                                        </div>
                                    )
                                ) : (
                                    <h4 className="text-lg font-black tracking-wide truncate leading-tight" style={{ color: 'var(--pp-text-primary)' }}>
                                        {intent.name}
                                    </h4>
                                )}

                                <div className="flex items-end gap-2 mt-1">
                                    <span className="text-3xl font-mono font-black text-amber-500">{intent.amount}</span>
                                    <span className="text-[10px] font-bold px-2 py-0.5 rounded border uppercase"
                                        style={{ color: 'var(--agt-blue)', background: 'rgba(27, 191, 236, 0.1)', borderColor: 'rgba(27, 191, 236, 0.25)' }}>
                                        {intent.token || 'alphaUSD'}
                                    </span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="mb-4">
                                {isWalletLocked ? (
                                    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                                        <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest">{'\u2713'} Wallet</span>
                                        <span className="text-xs font-mono text-emerald-600/70 truncate">{intent.wallet}</span>
                                    </div>
                                ) : (
                                    <div className="flex flex-col gap-1.5">
                                        <div className="flex items-center gap-1.5">
                                            <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse shrink-0"></span>
                                            <span className="text-[9px] font-bold tracking-[0.15em] text-amber-500 uppercase">Not in contacts — assign wallet</span>
                                        </div>
                                        <div className="relative flex items-center">
                                            <input
                                                type="text"
                                                ...
                                                className={`intent-wallet-input w-full border rounded-lg pl-3 pr-[3.75rem] py-[7px] text-sm focus:outline-none font-mono tracking-wide transition-all duration-200
                                                    ${walletInput && !walletInputValid
                                                        ? 'border-red-500/40 focus:border-red-400/60'
                                                        : walletInputValid
                                                            ? 'border-emerald-500/40 focus:border-emerald-400/60'
                                                            : 'border-amber-500/30 hover:border-amber-500/50 focus:border-amber-400/70'
                                                    }`}
                                                style={{ background: 'var(--pp-bg-elevated)', color: 'var(--pp-text-primary)' }}
                                            />
                                            <button
                                                ...
                                                className={`absolute right-1.5 top-1/2 -translate-y-1/2 text-[9px] font-bold uppercase tracking-widest px-2 py-[5px] rounded-md border transition-all duration-200
                                                    ${walletInputValid
                                                        ? 'text-emerald-500 bg-emerald-500/15 border-emerald-500/40 hover:bg-emerald-500/25 hover:border-emerald-400/70 cursor-pointer'
                                                        : 'bg-white/5 border-white/10 cursor-not-allowed'}`}
                                                style={!walletInputValid ? { color: 'var(--pp-text-muted)' } : {}}
                                            >
                                                Lock
                                            </button>
                                        </div>
                                        <p className="text-[9px] font-mono leading-none">
                                            {walletInput && !walletInputValid
                                                ? <span className="text-red-500">Invalid address — must be 0x + 40 hex chars</span>
                                                : walletInputValid
                                                    ? <span className="text-emerald-500/60">{'\u21B5'} Enter or Lock to confirm wallet</span>
                                                    : <span style={{ color: 'var(--pp-text-muted)' }}>Paste or type the recipient wallet address</span>}
                                        </p>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="pt-3 border-t border-dashed" style={{ borderColor: 'var(--pp-border)' }}>
                            <div className="flex items-center gap-2">
                                <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'var(--pp-text-muted)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg>
                                <input type="text" placeholder="On-chain memo (visible to recipient)" value={cardNotes[intent.indexId] || intent.note || ''} onChange={(e) => handleNoteChange(intent.indexId, e.target.value)}
