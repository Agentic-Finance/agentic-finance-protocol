                    ...
                    <div key={intent.indexId} className={`relative min-w-[340px] p-5 rounded-2xl border bg-[#0F1319] flex flex-col bg-[#061214]/90 shadow-lg ${cardBorderClass} group/card`}>
                        ...
                        <div className="pt-3 border-t border-white/5 border-dashed">
                            <div className="flex items-center gap-2">
                                <svg className="w-3.5 h-3.5 text-white/20 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg>
                                <input type="text" placeholder="On-chain memo (visible to recipient)" value={cardNotes[intent.indexId] || intent.note || ''} onChange={(e) => handleNoteChange(intent.indexId, e.target.value)} className="bg-transparent text-xs text-slate-400 w-full outline-none font-mono placeholder:text-white/15 focus:text-white/70" />
                            </div>
                        </div>
                    </div>
                );
