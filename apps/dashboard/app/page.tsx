...
export default function Dashboard() {
    ...
    const onboardingRef = useRef(false); // Block workspace auto-set during onboarding steps 4-6
    ...
    const toastComponent = (
        <div role="status" aria-live="polite" aria-atomic="true" className={`fixed bottom-10 right-0 left-0 md:left-auto md:right-10 z-[500] transition-all duration-500 ease-out transform flex justify-center md:justify-end pointer-events-none ${toast.show ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-95'}`}>
            <div className={`relative overflow-hidden flex items-start gap-4 p-5 pr-12 rounded-2xl border shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8)] ${toast.show ? 'pointer-events-auto' : 'pointer-events-none'} max-w-sm w-11/12 md:w-[400px] will-change-transform ${toast.type === 'success' ? 'bg-[#061A11]/95 border-emerald-500/40' : 'bg-[#1A060A]/95 border-rose-500/40'}`}>
    ...
    if (showLanding) { return <>{toastComponent}<Suspense fallback={<LazyFallback />}><LandingPage onLaunchApp={() => { setShowLanding(false); sessionStorage.setItem('agtfi_session_active', 'true'); }} /></Suspense></>; }
                    ...
                    steps={[
                        {
                            ...
                            action: () => { setShowFundInput(true); window.scrollTo({ top: 0, behavior: 'smooth' }); },
                            ...
                            spotlight: {
                                selector: 'onboard-vault',
                                title: 'Fund Your Vault',
                                description: 'This is your Platform Vault. Click "+ Top up" to deposit USDC or alphaUSD. Your vault balance powers all payments, streaming payroll, and escrow transactions.',
                                position: 'bottom',
                            },
                        },
                        {
                            ...
                            spotlight: {
                                selector: 'onboard-terminal',
                                title: 'Add Recipients via Omni Terminal',
                                description: 'Type a natural language command like "add 0x1234... as Tony" or use the CSV upload to bulk-import recipients. The AI will parse your intent automatically.',
                                position: 'bottom',
                            },
                        },
                        {
                            ...
                            spotlight: {
                                ...
                                description: 'Type something like "pay @Tony 10 AlphaUSD" in the terminal. The AI parses your command, creates an intent card, and you confirm with one click to execute on-chain.',
                                position: 'bottom',
                            },
                        },
                        {
                            ...
                            spotlight: {
                                selector: 'onboard-boardroom',
                                title: 'Enable ZK Shield Privacy',
                                description: 'Toggle the "Phantom Shield" switch in the Boardroom. This wraps your payments in zero-knowledge PLONK proofs — recipients get paid, but amounts and addresses stay private on-chain.',
                                position: 'top',
                            },
                        },
                        {
                            ...
                            action: () => { onboarding.completeStep('exploreAgents'); },
                            ...
                            spotlight: {
                                selector: 'onboard-agents',
                                title: 'AI-Powered Autopilot Agents',
                                description: 'Create rules like "Pay all pending invoices every Friday" or "Auto-approve payments under 50 USDC." These AI agents monitor your workspace and execute payments autonomously.',
                                position: 'top',
                            },
                        },
                    ]}
