/**
 * Agentic Finance MCP Server v2.1
 *
 * Exposes ALL Agentic Finance agents as MCP (Model Context Protocol) tools,
 ...
 *
 * Usage:
 *   AGTFI_AGENT_API=http://localhost:3001 node dist/index.js
 *   Then add to Claude Desktop's MCP config.
 *
 * Protocol: https://modelcontextprotocol.io
 * APS-1: https://agt.finance/aps-1
 */
...
const DASHBOARD_API = process.env.AGTFI_DASHBOARD_API ?? 'http://localhost:3000';
...
const CORE_TOOLS = [
  ...
  {
    name: 'agtfi_hire_agent',
    description: 'Hire any Agentic Finance agent by ID. This is the universal tool for executing agent tasks. Supports 32+ specialized agents including: contract-auditor, token-transfer, escrow-manager, payroll-planner, shield-executor, stream-creator, proof-verifier, coordinator-agent, balance-scanner, token-deployer, and more. Use agtfi_list_agents to discover available agents.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'agtfi_list_agents',
    description: 'List all available Agentic Finance agents with their capabilities, pricing, and categories. Use this to discover which agents can handle a task.',
    inputSchema: {
      type: 'object',
      properties: {
...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<string> {
  switch (name) {
    ...
    case 'agtfi_settle_escrow': {
      const prompt = `Settle escrow job ID ${args.job_id}`;
      return JSON.stringify(await executeAgent('escrow-manager', prompt, args), null, 2);
    }

    case 'agtfi_verify_proof': {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    return {
      content: [{ type: 'text', text: `Agentic Finance error: ${err.message}` }],
      isError: true,
    };
  }
});
...
server.connect(transport).then(() => {
  process.stderr.write('[agtfi-mcp v2.1] Server ready — APS-1 Agent Payment Standard\n');
  process.stderr.write(`[agtfi-mcp] Agent API: ${AGENT_API}\n`);
  process.stderr.write(`[agtfi-mcp] Tools: ${CORE_TOOLS.length} tools available\n`);
});
