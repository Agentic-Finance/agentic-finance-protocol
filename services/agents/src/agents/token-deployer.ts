...
export const handler: AgentHandler = async (job) => {
  ...
  try {
    // ── Phase 1: AI Design ──────────────────────────────────
    console.log(`[token-deployer] 🧠 Phase 1: AI designing token for: "${requirements.slice(0, 80)}..."`);

    const rawText = await aiComplete(SYSTEM_PROMPT, requirements, { maxTokens: 2048 });
    let design: any;
    try {
